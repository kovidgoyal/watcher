...
try:
    ...
    raise SystemExit('Run this script as fontforge -script /path/to/font/to/patch')

# Handle command-line arguments
parser = argparse.ArgumentParser(
    description='Font patcher for Kovid. Requires FontForge with Python bindings. Stores the patched font as a new, renamed font file by default.')
parser.add_argument('target_fonts', help='font files to patch', metavar='font', nargs='+', type=argparse.FileType('rb'))
parser.add_argument('--no-rename', help='don\'t add " for Kovid" to the font name', default=True, action='store_false', dest='rename_font')
parser.add_argument('--source-font', help='source symbol font', metavar='font', dest='source_font',
                    default='{0}/fontpatcher-symbols.sfd'.format(os.path.dirname(os.path.abspath(__file__))), type=argparse.FileType('rb'))
parser.add_argument('--dest-dir', help='Output directory', metavar='destdir', dest='destdir',
                    default=os.getcwd())
...
class FontPatcher(object):
    ...
    def __init__(self, source_font, target_fonts, destdir, rename_font=True):
    ...
    def patch(self):
        for target_font in self.target_fonts:
            ...
            if extension.lower() not in ['.ttf', '.otf']:
                ...
                extension = '.otf'
            target_font.generate(os.path.join(self.destdir, '{0}{1}'.format(target_font.fullname, extension)))

fp = FontPatcher(args.source_font, args.target_fonts, args.destdir, args.rename_font)
fp.patch()
